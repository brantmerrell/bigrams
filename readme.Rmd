...
### Executability
An R shiny application is available at [https://brantmerrell.shinyapps.io/programming_exercise/](https://brantmerrell.shinyapps.io/programming_exercise/), which allows users to manually upload one txt file at a time and view the resulting histogram of bigrams. The shiny application can also be run from the R console:  
```{r, eval=F}
library(shiny)
runApp()
```
...
bigram("<txt_file>") # generate file.png
```

### Issues to Fix
Review of the histograms generated (my thanks to MD Weight) calls attention to a few issues with the histograms. 
 * __The sum of the histogram's bars ought to be equal to 1__. This requires a normalization of the data (ie. divide each tally by the total number of bigrams) prior to calling the plot function, similar to a probability of mass distribution.  
 ...
 * __Bigrams with a tally of 1 take up unnecessary space__. “When a bigram count falls below the threshold t, the bigram is backed-off to the unigram probability suitably scaled by a back-off weight in order to ensure that all bigram probabilities for a given history sum to one.” - [Bigram Language Models](https://labrosa.ee.columbia.edu/doc/HTKBook21/node132.html).  
